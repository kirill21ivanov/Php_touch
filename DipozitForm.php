            ...
            *  $sender->sendMessage("§7Ваш счёт дипозита состовляет §b" . $bank[$player->getName()]["dipozit"]);
					  ...
					  *             $this->ra->save();
						*/
						if($gm >= $data[0]){
							if($data[0] != ""){
					  	  $pl->sendMessage("§l§3Вы §l§aуспешно §l§3сняли с депозита");
					  	  $pl->sendMessage("§7Чек на операцию: \nПеревод с депозита - §a{$data[0]}§7$");
						...
						}else $pl->sendMessage("§7У вас нехватает средств на депозите.");
				} else $this->open($pl);
	        });
